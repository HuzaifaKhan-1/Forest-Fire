...
// // NeuroNix AI Engine Configuration
const ML_API_BASE = 'http://127.0.0.1:5001';
console.log('🛰️ AI Engine V5.0 Handshake: SUCCESS at', ML_API_BASE);
let mlPredictions = {};
...
function initializeMaps() {
    ...
    try {
        ...
        if (riskMapElement) {
            riskMap = L.map('risk-map').setView([20.5937, 78.9629], 5);
        ...
        if (simulationMapElement) {
            simulationMap = L.map('simulation-map').setView([20.5937, 78.9629], 5);
        ...
        if (deploymentMapElement) {
            deploymentMap = L.map('deployment-map').setView([20.5937, 78.9629], 4);
        ...
        // Fetch and plot NASA FIRMS real-time fire data
        fetchAndPlotNASAData();
...
// Fetch and plot NASA FIRMS real-time fire data
async function fetchAndPlotNASAData() {
    if (!riskMap && !simulationMap) return;

    try {
        console.log('Fetching NASA FIRMS data...');
        //showToast('Syncing real-time satellite data (NASA FIRMS)...', 'info');
        
        const response = await fetch(`${ML_API_BASE}/api/ml/firms`);
        if (!response.ok) throw new Error('Failed to fetch FIRMS data');
        
        const data = await response.json();
        
        if (data.success && data.hotspots && data.hotspots.length > 0) {
            console.log(`Plotting ${data.hotspots.length} active fires from NASA FIRMS`);
            //showToast(`Plotted ${data.hotspots.length} active fires from NASA FIRMS`, 'success');
            
            // Create a custom fire icon
            const fireIcon = L.divIcon({
                className: 'nasa-fire-icon',
                html: '<i class="fas fa-fire" style="color: #ff4444; font-size: 14px; text-shadow: 0 0 5px #ff0000;"></i>',
                iconSize: [20, 20],
                iconAnchor: [10, 10]
            });

            // Group markers to easily toggle or update
            const hotspotsGroup = L.layerGroup();
            
            data.hotspots.forEach(hotspot => {
                const marker = L.marker([hotspot.latitude, hotspot.longitude], { icon: fireIcon });
                marker.bindPopup(`
                    <div class="fire-hotspot-popup">
                        <h4 style="color: #ff4444; margin-bottom: 5px;"><i class="fas fa-satellite"></i> NASA FIRMS Hotspot</h4>
                        <p><strong>Confidence:</strong> ${hotspot.confidence}%</p>
                        <p><strong>Brightness:</strong> ${hotspot.brightness} K</p>
                        <p><strong>Acquired:</strong> ${hotspot.acq_date} ${hotspot.acq_time}</p>
            ...
            // Add to maps
            if (riskMap) hotspotsGroup.addTo(riskMap);
            if (simulationMap) hotspotsGroup.addTo(simulationMap);
            
            // Try to update activity feed if present
                ...
                const newActivityHtml = `
                    <div class="activity-item new">
                        ...
                        <div class="activity-content">
                            <div class="activity-title">NASA FIRMS Sync</div>
                            <div class="activity-description">Plotted ${data.hotspots.length} live hotspots via MODIS</div>
...
async function updateRegionalCards() {
    try {
        const response = await fetch(`${ML_API_BASE}/api/ml/realtime?region=all`);
        if (!response.ok) throw new Error('API unavailable');
        const data = await response.json();

        if (data.success && data.predictions) {
            Object.keys(data.predictions).forEach(region => {
                ...
                if (card) {
                    ...
                    const ml = prediction.prediction;

                    // Update risk badge and class
                    const badge = card.querySelector('.risk-badge');
                    const riskPercent = card.querySelector('.risk-percentage');
                    const riskLabel = card.querySelector('.region-stats b span');

                    const riskCategory = ml.risk_category || 'moderate';
                    card.className = `region-card ${riskCategory}-risk`;

                    if (badge) {
                        badge.className = `risk-badge ${riskCategory}`;
                        badge.textContent = riskCategory.replace('-', ' ').toUpperCase();
                    }

                    if (riskPercent) {
                        riskPercent.textContent = Math.round(ml.overall_risk * 100) + '%';
                    }
...
async function updateEnvironmentalData() {
    try {
        const response = await fetch(`${ML_API_BASE}/api/ml/realtime?region=Nainital`);
        if (!response.ok) throw new Error('API unavailable');
        const data = await response.json();
...
function updateEnvironmentalConditions() {
    ...
    if (temperatureEl) {
        ...
        temperatureEl.textContent = newTemp + 'Â°C';
    }
...
function initializeMLIntegration() {
    ...
    showToastOnce('ml_init', 'AI/ML models initialized successfully', 'success');
}

async function startMLRealTimeUpdates() {
    try {
        let apiUrl = `${ML_API_BASE}/api/ml/start-realtime`;
        // Force port 5001 if using localhost/127.0.0.1
        if (apiUrl.includes('127.0.0.1:5500') || apiUrl.includes('localhost:5500')) {
            apiUrl = apiUrl.replace(':5500', ':5001');
        }
        
        const response = await fetch(apiUrl, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
        }).catch(() => fetch(`http://127.0.0.1:5001/api/ml/start-realtime`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
        }));

        if (response && response.ok) { // Check if response is defined before accessing .ok
            realTimeUpdates = true;
            showToastOnce('ml_realtime', 'Real-time AI predictions activated', 'success');
        }
    } catch (error) {
...
async function loadMLModelInfo() {
    try {
        const response = await fetch(`${ML_API_BASE}/api/ml/model-info`)
            .catch(() => fetch(`http://127.0.0.1:5001/api/ml/model-info`));
...
async function initializeExplainabilityMap() {
    const mapElement = document.getElementById('explainability-map');
    if (!mapElement) return;

    try {
        explainabilityMap = L.map('explainability-map').setView([29.3806, 79.4422], 10);

        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
            attribution: 'Â© OpenStreetMap contributors'
        }).addTo(explainabilityMap);

        // Fetch real-time data for the region (e.g., Nainital)
        const response = await fetch(`${ML_API_BASE}/api/ml/realtime?region=Nainital`);
        const data = await response.json();
        const env = data.success ? data.prediction.environmental_data : null;

        addFireSourceWithExplanation(explainabilityMap, [29.3806, 79.4422], env);
        addWindVectors(explainabilityMap, env);
        addVegetationLayers(explainabilityMap, env);
...
function addFireSourceWithExplanation(map, latlng, env) {
            ...
            html: `
                <div class="fire-source-container">
                    ...
                    <div class="explanation-indicators">
                        <div class="wind-indicator" title="Wind Impact">${env ? (env.wind_speed > 20 ? 'ðŸ”¥' : 'ðŸŒªï¸') : 'ðŸŒªï¸'}</div>
                        <div class="dryness-indicator" title="Humidity/Dryness">${env ? (env.humidity < 30 ? 'ðŸ”¥' : 'ðŸŒ±') : 'ðŸŒ±'}</div>
                        <div class="slope-indicator" title="Terrain Impact">â›°ï¸</div>
                    </div>
                </div>
            `,
            iconSize: [60, 60],
    ...
    fireMarker.on('click', function () {
        showFireExplanationPopup(latlng, env);
    });
...
function addWindVectors(map, env) {
    const windSpeed = env ? Math.round(env.wind_speed) : 22;
    const windDir = env ? env.wind_direction : 'NE';

    // Map compass to degrees
    const dirMap = { 'N': 0, 'NE': 45, 'E': 90, 'SE': 135, 'S': 180, 'SW': 225, 'W': 270, 'NW': 315 };
    const windDeg = dirMap[windDir] || 45;

    const windVectors = [
        { pos: [29.3906, 79.4522], direction: windDeg, strength: windSpeed },
        { pos: [29.3706, 79.4322], direction: windDeg + (Math.random() * 10 - 5), strength: Math.max(0, windSpeed - 2) },
    ];
    ...
    windVectors.forEach(vector => {
            ...
            icon: L.divIcon({
                ...
                html: `
                    <div class="wind-arrow" style="transform: rotate(${vector.direction}deg)">
                        <div class="arrow-body" style="height: ${Math.min(40, vector.strength * 2)}px"></div>
        ...
        windMarker.bindPopup(`
            <div class="wind-explanation">
                <h4>Dynamic Wind Analysis</h4>
                <p>Speed: ${vector.strength} km/h</p>
                <p>Direction: ${windDir} (${vector.direction}Â°)</p>
                <p>Impact: ${vector.strength > 15 ? 'Critical driver' : 'Moderate influence'} of fire spread.</p>
            </div>
        `);
    });
}
...
function initializeTimelineControls() {
    ...
    const timelineExplanations = [
        ...
        "Fire encounters 20Â° slope. Uphill spread rate doubles.",
        "Temperature peaks at 36Â°C. Maximum fire intensity reached.",
...
function initializeWhatIfControls() {
    ...
    updateSliderValue('slope-value', slopeSlider, 'Â°');
    ...
    if (slopeSlider) {
        slopeSlider.addEventListener('input', (e) => {
            updateSliderValue('slope-value', e.target, 'Â°');
...
function updateWhatIfExplanation(windSpeed, humidity, slope, windDirection) {
    ...
    if (parseInt(slope) > 25) {
        explanation += `Steep terrain (${slope}Â°) will cause rapid uphill fire advancement.`;
    }
...
function resetWhatIfControls() {
    ...
    document.getElementById('slope-value').textContent = '15Â°';
...
function showDetailedFactorExplanation(factor) {
    const explanations = {
        ...
        slope: {
            ...
            description: '15Â° upward slope significantly enhances fire spread rate.',
            details: [
...
function updateLiveFactors() {
    ...
    const baseValues = {
        ...
        'live-temp': { value: 34, unit: 'Â°C', variance: 2 },
        'live-slope': { value: 15, unit: 'Â°', variance: 0 }
    };
...
// Toast Notifications
function showToastOnce(key, message, type = 'success', duration = 3000) {
    if (!sessionStorage.getItem('toast_' + key)) {
        showToast(message, type, duration);
        sessionStorage.setItem('toast_' + key, 'true');
    }
}
...
class FireVision3D {
    ...
    showFireInfoPopup(point) {
        ...
        document.getElementById('popup-slope').textContent = slope.toFixed(1) + 'Â°';
        document.getElementById('popup-temperature').textContent = temperature.toFixed(1) + 'Â°C';
    ...
    createFireFlames(fireSource) {
        ...
        for (let i = 0; i < flameCount; i++) {
            ...
            const temp = 500 + Math.random() * 600; // 500-1100Â°C
    ...
    showExplanationLabels() {
        ...
        const factors = [
            { text: 'ðŸ”¥ Wind', position: new THREE.Vector3(-20, 15, -10), color: 0x60A5FA },
            { text: 'ðŸŒ± Dryness', position: new THREE.Vector3(15, 12, 20), color: 0x22C55E },
            { text: 'â›°ï¸ Slope', position: new THREE.Vector3(0, 20, 0), color: 0x92400E }
        ];
...
function initializeEvacuationRoutes() {
    ...
    if (evacuationMapElement) {
        ...
        showToastOnce('evac_init', 'Evacuation route system initialized', 'success');
    }
}
...
function initializeCommunityEngagement() {
    ...
    showToastOnce('community_load', 'Community engagement module loaded successfully!', 'success');
}
...
function startTrainingSimulation() {
    ...
    showToast('ðŸ”¥ Fire emergency started! Protect the village!', 'success', 3000);
    ...
    if (fireSource && village) {
        ...
        if (newSize > 1.5 && !document.querySelector('.village-protected')) {
            ...
            if (Math.random() < 0.1) { // 10% chance per second when close
                showToast('âš ï¸ Fire is approaching the village!', 'warning', 2000);
            }
        }
    }
}
...
function levelUp() {
    ...
    showToast('ðŸŽ‰ Level Up! You are now a Fire Specialist!', 'success', 4000);
}
...
function unlockRandomBadge() {
    ...
    if (lockedBadges.length > 0) {
        ...
        showToast(`ðŸ† Badge Unlocked: ${badgeName}!`, 'success', 3000);
...
function initializeEnvironmentalImpact() {
    ...
    showToastOnce('env_ready', 'Environmental impact analysis ready', 'success');
}
    ...
    if (carbonCtx) {
        window.carbonEmissionsChart = new Chart(carbonCtx.getContext('2d'), {
            ...
            data: {
                labels: ['0h', '1h', '2h', '3h', '4h', '5h', '6h'],
                datasets: [{
                    label: 'COâ‚‚ Emissions (tonnes/hour)',
                    data: [0, 0, 0, 0, 0, 0, 0],
                    borderColor: '#F59E0B',
            ...
            options: getChartOptions('COâ‚‚ Emissions Over Time', 'tonnes/hour')
        });
    }
    ...
    if (carbonLossCtx) {
        window.carbonLossChart = new Chart(carbonLossCtx.getContext('2d'), {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Lost Sequestration (tonnes COâ‚‚)',
                    data: [0, 0, 0, 0, 0],
            ...
            options: getChartOptions('Carbon Sequestration Loss Over Time', 'tonnes COâ‚‚')
        });
    }
}
...
function updateCarbonEmissionsDisplay(emissions) {
    ...
    // Update emissions chart with simulation data
    if (window.carbonEmissionsChart && typeof isSimulationRunning !== 'undefined' && isSimulationRunning) {
        const hours = Math.min(simulationTime / 10, 6); // Scale simulation time to hours
        const hourlyRate = emissions.total_co2_emissions_tonnes / Math.max(hours, 1);
...
function exportAnalysisReport() {
    ...
    const area = document.getElementById('analysis-area')?.value || 0;
    const vegetationType = document.getElementById('vegetation-type')?.value || 'N/A';
    const severity = document.getElementById('fire-severity')?.value || 'N/A';
    const reportContent = `
...
async function dispatchDemoSMS() {
    const phoneInput = document.getElementById('demo-phone-number')?.value.trim();
    const messageInput = document.getElementById('demo-custom-message')?.value.trim();
    if (!phoneInput) {
        showToast('Please enter a valid phone number with country code (e.g., +91...)', 'error', 3000);
        return;
    }
    showToast('Dispatching Emergency SMS...', 'info', 2000);
    try {
        const response = await fetch('http://127.0.0.1:5001/api/ml/send-alert', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                phone_number: phoneInput,
                location: 'Sector 4, Near Nainital Highway',
                risk_level: 'Critical',
        ...
        if (response.ok && data.success) {
            showToast('🚨 Alert Sent successfully!', 'success', 3000);
            const activityFeed = document.querySelector('.activity-feed');
            if (activityFeed) {
                const newActivityHtml = `
                    <div class="activity-item new">
                        <div class="activity-icon" style="background: rgba(255, 68, 68, 0.2); color: #ff4444;">
                            <i class="fas fa-satellite-dish"></i>
                        </div>
                        <div class="activity-content">
                            <div class="activity-title" style="color: #ff4444;">Emergency SMS Dispatched</div>
                            <div class="activity-description">Alert sent to ${phoneInput}</div>
                            <div class="activity-time">Just now</div>
                        </div>
                    </div>
                `;
            ...
            showToast('Failed to send SMS: ' + (data.error || 'Unknown error'), 'error', 4000);
        }
    } catch (error) {
        console.error('Error sending SMS:', error);
        showToast('Error connecting to alert service. Is the backend running?', 'error', 4000);
    }
}
...
document.addEventListener('keydown', function (e) {
    ...
    if ((e.ctrlKey || e.metaKey) && e.key === 'e') {
        e.preventDefault();
        if (typeof exportAnalysisReport === 'function') exportAnalysisReport();
    }
...
async function updateDashboardExplainability() {
    const drynessWeightEl = document.getElementById('weight-dryness');
    const fillDrynessEl = document.getElementById('fill-dryness');
    const windWeightEl = document.getElementById('weight-wind');
    const fillWindEl = document.getElementById('fill-wind');
    const slopeWeightEl = document.getElementById('weight-slope');
    const fillSlopeEl = document.getElementById('fill-slope');
    const tempWeightEl = document.getElementById('weight-temp');
    const fillTempEl = document.getElementById('fill-temp');
    const reasonTextEl = document.getElementById('ai-explanation-text');
    const confidenceEl = document.getElementById('explain-confidence');

    if (!drynessWeightEl) return;

    try {
        const windSpeed = parseInt(document.getElementById('live-wind')?.textContent) || 22;
        const humidity = parseInt(document.getElementById('live-humidity')?.textContent) || 32;
        const temperature = parseInt(document.getElementById('live-temp')?.textContent) || 34;

        const response = await fetch(`${ML_API_BASE}/api/ml/explain_v2`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                wind_speed: windSpeed,
        ...
        if (data.success) {
            ...
            if (drynessWeightEl) drynessWeightEl.textContent = `${weights.dryness}%`;
            if (fillDrynessEl) fillDrynessEl.style.width = `${weights.dryness}%`;
            if (windWeightEl) windWeightEl.textContent = `${weights.wind}%`;
            if (fillWindEl) fillWindEl.style.width = `${weights.wind}%`;
            if (slopeWeightEl) slopeWeightEl.textContent = `${weights.slope}%`;
            if (fillSlopeEl) fillSlopeEl.style.width = `${weights.slope}%`;
            if (tempWeightEl) tempWeightEl.textContent = `${weights.temperature}%`;
            if (fillTempEl) fillTempEl.style.width = `${weights.temperature}%`;
            if (reasonTextEl) reasonTextEl.textContent = data.explanation;
            if (confidenceEl) confidenceEl.textContent = `${data.confidence_score}%`;
        }
    } catch (error) {
