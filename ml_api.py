...
def get_config():
    """Serve configuration to frontend (e.g. Mapbox token)"""
    return jsonify({
...
def root_health_check():
    return "NeuroNix AI Engine is online and responding on port 5001! Use /api/ml/firms for data."
...
class RealTimePredictor:
    ...
    def _prediction_loop(self):
        ...
        regions = [
            'Nainital', 'Almora', 'Shimla', 'Wayanad', 'Similipal', 
            'Bandipur', 'Kanha', 'Gir', 'Kaziranga', 'Pachmarhi'
        ]
    ...
    def _generate_regional_data(self, region: str) -> dict:
        """Fetch actual real-time environmental data for a region using Open-Meteo API"""
        coordinates = {
            'Nainital': {'lat': 29.39, 'lon': 79.45, 'elev': 2084},
            'Almora': {'lat': 29.59, 'lon': 79.65, 'elev': 1642},
            'Shimla': {'lat': 31.10, 'lon': 77.17, 'elev': 2276},
            'Wayanad': {'lat': 11.68, 'lon': 76.13, 'elev': 700},
            'Similipal': {'lat': 21.93, 'lon': 86.35, 'elev': 500},
            'Bandipur': {'lat': 11.66, 'lon': 76.62, 'elev': 800},
            'Kanha': {'lat': 22.33, 'lon': 80.61, 'elev': 600},
            'Gir': {'lat': 21.14, 'lon': 70.82, 'elev': 150},
            'Kaziranga': {'lat': 26.66, 'lon': 93.36, 'elev': 60},
            'Pachmarhi': {'lat': 22.46, 'lon': 82.43, 'elev': 1000}
        }
        
        coords = coordinates.get(region, {'lat': 29.39, 'lon': 79.45, 'elev': 1500})
...
@app.route('/api/ml/firms', methods=['GET'])
def get_firms_data():
    """Fetch real-time active fire hotspots from NASA FIRMS"""
    try:
        # Fetch South Asia 24h data (open access, no API key needed)
        url = "https://firms.modaps.eosdis.nasa.gov/data/active_fire/modis-c6.1/csv/MODIS_C6_1_South_Asia_24h.csv"
        response = requests.get(url, timeout=10)
        
        if response.status_code != 200:
            return jsonify({'success': False, 'error': 'Failed to fetch FIRMS data'}), 500
            
        # Parse CSV
        df = pd.read_csv(io.StringIO(response.text))
        
        # We only really need lat, lon, confidence, brightness
        # India bounding box roughly: lat 8 to 37, lon 68 to 97
        df_india = df[
        ...
        hotspots = df_india[['latitude', 'longitude', 'confidence', 'brightness', 'acq_date', 'acq_time']].to_dict(orient='records')
...
@app.route('/api/ml/explain_v2', methods=['POST'])
def explain_fire_behavior_v2():
    """API endpoint for AI explainability v2"""
    try:
        data = request.get_json()
...
@app.route('/api/ml/whatif_v2', methods=['POST'])
def whatif_simulation_v2():
    """API endpoint for What-If scenario testing v2"""
    try:
        data = request.get_json()
...
@app.route('/api/ml/replay-explanation-v2', methods=['POST'])
def replay_explanation_v2():
    """API endpoint for step-by-step fire spread explanation v2"""
    try:
        data = request.get_json()
...
@app.route('/api/ml/send-alert', methods=['POST'])
def send_sms_alert():
    ...
    try:
        ...
        if not all([TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_PHONE_NUMBER]):
            # If credentials aren't set, simulate success for demonstration/hackathon purposes
            # print("Warning: Twilio credentials not set. Simulating SMS success.")
            return jsonify({
                'success': True,
                'message': 'Simulated SMS sent successfully (No Twilio config)',
                'details': {
                    'to': phone_number,
                    'status': 'simulated',
                    'note': 'Set TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, and TWILIO_PHONE_NUMBER to send real SMS'
                }
            })
        ...
        # Build message body
        if message_custom:
            body = f"NeuroNix Alert: {message_custom}"
        else:
            body = f"🚨 NEURONIX EMERGENCY ALERT 🚨\nRisk Level: {risk_level.upper()}\nLocation: {location}\nImmediate action may be required. Follow local authorities' guidance."
...
@app.route('/api/ml/start-realtime', methods=['POST', 'OPTIONS'])
def start_realtime():
